...
shared_ptr<Preprocess> p_pre(new Preprocess()); //智能指针，不需要delete，没有托管后自动释放
...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg) 
{
    ...
    p_pre->process(msg, ptr); // 特征提取或间隔采样
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in) 
{
    ...
    // lidar和imu时间相差过大，且开启时间同步, 纠正当前输入imu的时间
    if (abs(timediff_lidar_wrt_imu) > 0.1 && time_sync_en)
    {
        //'\'反斜杠代表换行
        //对输入的imu时间，纠正为 时间差 + 原始时间
        msg->header.stamp = \
...
double lidar_mean_scantime = 0.0; //一帧点云平均扫描时间
int    scan_num = 0;
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    if(!lidar_pushed)
    {
        meas.lidar = lidar_buffer.front(); //lidar数据存入meas.lidar中
        ...
        //最后一个点的时间 小于 单帧扫描时间的一半
        else if (meas.lidar->points.back().curvature / double(1000) < 0.5 * lidar_mean_scantime)
        {
            // 记录lidar结束时间为 起始时间 + 单帧扫描时间
            lidar_end_time = meas.lidar_beg_time + lidar_mean_scantime;
        }
        else
        {
    ...
    while ((!imu_buffer.empty()) && (imu_time < lidar_end_time))
    {
        ...
        meas.imu.push_back(imu_buffer.front()); //imu数据存入meas.imu中
        imu_buffer.pop_front();
    }

    lidar_buffer.pop_front();
    time_buffer.pop_front();
    lidar_pushed = false; //imu提取结束后才允许雷达进行下一次提取
    return true;
}
...
int main(int argc, char** argv)
{
    ...
    double epsi[23] = {0.001}; //只有第一项设为0.001
    fill(epsi, epsi+23, 0.001); //全部填充为0.001
...
//------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        if (flg_exit) break;
        ros::spinOnce();
        if(sync_packages(Measures)) //提取一帧lidar特征点云和该帧持续扫描时间内对应的imu数据
        {
            //如果是第一帧lidar数据
            if (flg_first_scan)
            {
            ...
            {
                ROS_WARN("No point, skip this scan! FAST-LIO not ready\n");
                continue;
            }
